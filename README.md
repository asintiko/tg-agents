...
- Для футбольных проектов первый слот дня — пост-прогноз по матчам (можно отключить в настройках).
- Статус воркера и очереди доступен по `/projects/{id}/autopost/status` и отображается в админке.
