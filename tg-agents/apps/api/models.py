...
class TelegramMode(str, enum.Enum):
    BOT = "bot"
    USER = "user"
...
class TelegramConnection(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    project_id: Mapped[int] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"))
    mode: Mapped[TelegramMode] = mapped_column(
        Enum(TelegramMode, values_callable=enum_values, name="telegrammode"), nullable=False
    )
    bot_token_encrypted: Mapped[str | None] = mapped_column(String(255), nullable=True)
