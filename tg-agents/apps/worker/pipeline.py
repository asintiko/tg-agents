...
from apps.api.telegram_user import send_user_message
...
class PostPipeline:
    ...
    async def _process_post(self, session: AsyncSession, post: Post) -> None:
        ...
        while attempts < 2:
            try:
                if connection.mode == TelegramMode.BOT:
                    if not connection.bot_token_encrypted:
                        raise RuntimeError("не задан токен бота")
                    token = self.settings.decrypt(connection.bot_token_encrypted)
                    await send_to_channels(token, chat_ids, text, image_path=image_path)
                    post.tg_message_id = "sent-bot"
                else:
    ...
    async def _get_connection(
        ...
        result = await session.execute(
            ...
            .where(TelegramConnection.project_id == project_id)
            .order_by(TelegramConnection.mode.desc())
        )
