...
async def create_project_with_data() -> tuple[int, int]:
    ...
    async with session_maker() as session:
        ...
        post = Post(
            ...
            planned_at=msk_now() - timedelta(minutes=1),
...
@pytest.mark.asyncio
async def test_pipeline_publishes_with_user_session(monkeypatch: pytest.MonkeyPatch) -> None:
    await setup_base()
    project_id, post_id = await create_project_with_data()
    gen_post = GeneratedPost(
        headline="h",
    ...
    async def fake_status(*args: Any, **kwargs: Any) -> dict[str, Any]:
        return {"status": "connected", "me": None, "last_connected_at": None}

    monkeypatch.setattr("apps.worker.pipeline.telegram_status", fake_status)
    pipeline = PostPipeline(
        get_sessionmaker(), generator=FakeGenerator(gen_post), image_service=FakeImageService()
    )
    await pipeline.run(project_id=project_id)
